var _pricingRender23 = function($elm, $type, $refresh) {
  ...
  var host = Bridge.Context.match('.feed_host', 'dev');
  ...
  var contextResponse = Bridge.Context.match(
    `.${storePath}-table-response`,
    []
  );
  ...
  function _tableFRender(e) {
    ...
    Bridge.Context.set(`${storePath}-table-response`, data);
      ...
      if (index === 0) {
        ...
        $pageContainer
          ...
          .text(
            `${
              value.data[0]
                ? '$' + addCommas(Math.round(value.data[0].currentRate))
                : '-'
            }`
          );
        $pageContainer
  ...
  function _initFRender() {
    ...
    $.each(uris, function(index, value) {
      if (supplements[index].sub !== 'Subscription') {
  ...
  function _initDropdownFilters(e) {
    var subscription, commitment, product, tier, duration;
    var state = 4;
    var tiers = [5, 15, 16, 17];
    ...
    if (subscription || commitment || product) {
      ...
      if (
        dropdownFilterStateData[$dropdownLabel.eq(state).text()] &&
        e === state
      ) {
        $pageContainer.find('[data-label="Tier"] [value]').hide();

        $.each(
          dropdownFilterStateData[$dropdownLabel.eq(state).text()],
          function(index, item) {
            $pageContainer.find(`[data-label="Tier"]`).removeClass('disabled');
            $pageContainer
              .find(`[data-label="Tier"] .input-label`)
              .text('Tier');
            $pageContainer
              .find(`[data-label="Tier"] [value="${item}"]`)
              .removeClass('strong')
              .show();
          }
        );
        ...
        $pageContainer.find(`[data-label="Tier"]`).removeClass('disabled');
        $pageContainer.find(`[data-label="Tier"] [value]`).show();
      }
    }
  }
