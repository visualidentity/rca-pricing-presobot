...
function toggleArrow(arrow, toggle) {
  if (toggle) {
    ...
    $(currentSlide() + ' .slide-footer .navbtn-target.' + arrow).removeClass(
      'inactive'
    );
  } else {
    $(currentSlide() + ' .slide-footer .navbtn.' + arrow).addClass('inactive');
...
function subslideIn(other) {
  ...
  var key = Bridge.Sub.current_subslide ? Bridge.Sub.current_subslide.key : '';
  ...
  if (downClicked) {
    ...
    if (slideDifference > 1) {
      ...
      for (var i = slideDifference; i > 1; i--) {
        ...
        var currentSubSlide = $(curSlide + prefix + (otherSlideID + i - 1));
        ...
        if (!nextSubSlide.hasClass('transparent-base')) {
          currentSubSlide.removeClass('fadeinslide').addClass('fadeoutslide');
        }
...
function subslideOut(other) {
  ...
  var thisSlideID = parseInt(this.el.id.split('-')[1]);
  var otherSlideID = parseInt(other.el.id.split('-')[1]);
  console.log('thisSlideID ' + thisSlideID + ' - otherSlideID ' + otherSlideID);
  ...
  if (downClicked) {
    ...
    if (slideDifference > 1) {
      ...
      for (var i = slideDifference; i > 1; i--) {
        ...
        var currentSubSlide = $(curSlide + prefix + (otherSlideID + i - 1));
        var previousSubSlide = $(curSlide + prefix + (otherSlideID + i));

        // Add subslide cross-fade
        // This allows for a smooth reverse animation when navigating back
        // Do not cross fade if next subslide has a transparent-base
        if (!previousSubSlide.hasClass('transparent-base')) {
          currentSubSlide.removeClass('fadeoutslide').addClass('fadeinslide');
        }
