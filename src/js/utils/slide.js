...
function attachSlideEvents(properties) {
  ...
  if (
    !(
      jumpToSlideID === window.currSlide && jumpFromSlideID === window.prevSlide
    )
  ) {
    window.jumpTo = '';
    window.jumpFrom = '';
...
function attachDisclaimerEvents(disclaimerSelector) {
  ...
  if (
    ...
    $(disclaimerSelector + ' .disclaimer-target').on('click', function() {
      ...
      if (
        $(disclaimerSelector + ' .slide-footer').hasClass('disclaimer-active')
      ) {
        Bridge.Event.trigger('slide:closeDisclaimer', disclaimerSelector);
      } else {
        Bridge.Event.trigger('slide:openDisclaimer', disclaimerSelector);
      }
    });
  ...
  Bridge.Event.on('slide:closeDisclaimer', function(selector) {
    ...
    $(selector + ' footer.disclaimer-active').removeClass('disclaimer-active');
  });
...
function attachPopupEvents(popupPage) {
  ...
  if ($('body').hasClass('master') || $('body').hasClass('share_online')) {
    ...
    $(popupPage + ' .overlay-btn').each(function(i) {
      ...
      var popupOverlay = {};
      popupOverlay.btn = popupPage + ' .overlay-btn.overlay-btn-' + (i + 1);
      popupOverlay.overlay = popupPage + ' .overlay.overlay-' + (i + 1);
      popupOverlay.disclaimer =
        popupPage + ' .overlay-disclaimer.overlay-disclaimer-' + (i + 1);
      popupOverlay.closeBtn =
        popupPage + ' .overlay.overlay-' + (i + 1) + ' .close-overlay-btn';
      popupOverlay.asset = popupPage + ' .overlay-asset.overlay-' + (i + 1);
