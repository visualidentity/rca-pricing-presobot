var brandingRender = function($pageContainer) {
  // find agent branding colour, or set default...
  // default = 'Biggin & Scott' colour
  var branding_colour = Bridge.Context.match(
    '.customer .primary_colour',
    '#00A4C7'
  );
  ...
  $pageContainer
    .find('.brand-bg')
    .css('background-color', branding_colour)
    .addClass(assetColourTest(branding_colour));
  ...
  $pageContainer
    .find('.contract-option-selector__header')
    .css('background-color', branding_colour)
    .addClass(assetColourTest(branding_colour));

  $pageContainer
    .find('.contract-option-selector__header')
    .css('border-top-color', branding_colour);

  if (branding_logo) {
    // update agent branding logo...
    $pageContainer
      .find('.brand-logo')
      .css('background-image', 'url(' + branding_logo + ')');
  } else {
